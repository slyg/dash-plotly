...
data_set_file = 'data/timeline_last_90_days.pkl'

df = pd.read_pickle(data_set_file)
creation_time = time.ctime(os.path.getctime(data_set_file))

with open('data/timeline_last_90_days.json') as json_file:
...
horizontal_axis_labels = [stats['date']['from'] for stats in all_days_stats]
...
layout = dict(
    title=go.layout.Title(text='Failure percentage of master jobs over the last {0} days<br>(generated on {1})'.format(days_in_past, creation_time),
